...
$(function(){
	$(".Add_tab_one").click(function(){
		localStorage.removeItem("team_fgId");
		window.location.href="add_items_qq.html";
	})
	var d = new Date();
	var y = d.getFullYear();
	var m = d.getMonth() + 1;
	...
	$(".time_ts").click(function(){
		if($(this).attr("flage") == "false"){
			...
			if($(".time_ts_one").val() != ""){
				$(".summary_time").css("display","block");
				$(".summary_time3").css("display","block");
	...
	$(".custom").on("click",function(event){
		...
		$(".summary_time").css("display","none");
		$(".summary_time1").css("display","none");
//		$(".time_ts").attr("id","appDateTime");
		datatime();
		$(".appDateTimetwo").click();
		$(".summary_time").css("display","none");
		$(".summary_time1").css("display","none");
//		$(".time_ts").removeAttr("id");
	})
	$(".custom2").on("click",function(event){
		event.stopPropagation();
		$(".summary_time").css("display","none");
		$(".summary_time2").css("display","none");
...
function ligth_all(){
	...
	$.ajax({
	      ...
	      success:function(json){
	    	  if(json.msg=="success"){
	    		  //自己
	    		  var myHtml ='<li><em><i style="display:none;">'+userId+'</i>';
	    		  myHtml +='<i style="display:none;">'+team_fgName+'</i>';
	    		  myHtml +='<i style="display:none;">'+team_fgLogo+'</i>';
	    		  myHtml +='</em> <img src="'+team_fgLogo+'" alt="" />';
	    		  myHtml +='<div class="Select_list"><span>'+team_fgName+'</span>';
	    		  myHtml +='<i>'+team_note+'</i></div></li>';
				$(".oneself").html(myHtml);
