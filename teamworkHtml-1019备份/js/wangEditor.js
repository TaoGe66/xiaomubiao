...
function Image(editor) {
    this.editor = editor;
    this.$elem = $('<div class="w-e-menu"><i class="w-e-icon-image"> 上传图片<i/></div>');
    this.type = 'panel';
...
Editor.prototype = {
    ...
    _initDom: function _initDom() {
        ...
        if (textSelector == null) {
            ...
            // 自行创建的，需要配置默认的样式
            $toolbarElem.css('position', 'absolute').css('top', '300px');
            $textContainerElem.css('border', '1px solid #ccc').css('height', '300px').css("margin-top","25px");
        } else {
