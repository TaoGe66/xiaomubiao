...
var init={
	...
	Ok:function(json, code,type){
		...
		if(json.msg=='success'){
			...
			isend=json.isend;
			if(type==1&&page==1){
				isShowRange=json.users.length>0?1:0;
...
var e={
	 show:function(json){
			...
			$(".Sign_people").html("创建人：&nbsp;"+json.userName);
	 ...
	 dShow:function(users,type){
	 	for(var x=0;x<users.length;x++){
	 		...
	 		var dhtml='<li><p class="userId" style="display: none;">'+user.userId+'</p>';
	 			...
	 			if(user.newPushRecord==0&&isEventDuty==0){
	 ...
	 */
	 	    ...
	 	    if(item.isReward!==1){
	 	    	...
	 	    	switch(item.state){
					...
					if(item.isAccept==2){
					 ...
					 if(item.state==2||item.isInvalid==1){
					...
					var cTime=getFormatDateByLong(Number(new Date().getTime()), "yyyy-MM-dd");
						...
						if(createBy==item.dutyId||isEventDuty==0){
								iHtml+='<div class="Event_icon BorderTop">';
							 	...
							 	if(editFlag&&isEventDuty==0&&(isToday(startTime)||cTime<starttimeHaoMiao)&&item.state!==2){
									iHtml+='<img src="img/edit_03.png" alt="" class="event_detailedPiece_publicSuo event_detailedPiece_Icon2 event_edit_icon">';
								  }
	 ...
	 hasShow:function(json){
		...
		if(json.comments==null){
			...
			json.isEventDuty==0?$(".Event_footer").show():$(".Event_footer").hide();
