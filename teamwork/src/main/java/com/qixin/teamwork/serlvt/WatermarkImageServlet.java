...
*
 */
public class WatermarkImageServlet extends HttpServlet {
   ...
   protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
       ...
       if (StrUtils.isNotEmpty(errorcode)) {
           ...
           try {
              ...
              /*if (CommConstants.FILETYPE.indexOf("|" + fileType + "|") < 0) {
            	   System.out.println(" 图片类型错误 4206123213123---------------------------------------------====================");
                   // 图片类型错误 4206
                   state="0";
                   errorCode=APIErrorMap.errorMap.get("9");
                   printNew(jsoncallback,state,errorCode,"", response);
                   return;
               }*/
               ...
               //图片生成水印处理,图片路径
             Thumbnails.of(model+imgUrl)  
       	    .size(1280,1024)
       	    .watermark(Positions.CENTER,ImageIO.read(new File(model+"watermarkater.png")),0.5f)
       	    .outputQuality(0.8f)  
       	    .toFile(model+imgUrl);
               ...
               e.printStackTrace();
               state="0";
               errorCode=APIErrorMap.errorMap.get("15");
               printNew(jsoncallback,state,errorCode,"",  response);
